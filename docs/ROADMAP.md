...
| T1 | ~~**Tool-call side-effect isolation**~~ **Done** — `io.github.rifatcakir.springai.testtools.recorder.tool`. Raised by an external review (`docs/EXTERNAL-FEEDBACK.md`) as a genuine gap: under `INSIDE_TOOL_LOOP`, a real `@Tool` method with a side effect (a DB write, an outbound API call) re-ran on *every* replay, forever. The reviewer's own proposed fix (proxy the `ToolCallback` interface via a `BeanPostProcessor`) was checked against real Spring AI 2.0.0 bytecode before being accepted, and it didn't hold up: `DefaultToolCallingManager.executeToolCall` resolves a callback first from the request's own `ToolCallingChatOptions` — exactly what `.tools(someObject)` produces, fresh per call, never a Spring bean — falling back to a bean-backed resolver only when that lookup misses, meaning a `ToolCallback`-level proxy would silently miss the dominant, README-documented usage pattern. The actual bean every invocation passes through, confirmed via bytecode and a working diagnostic probe: `ToolCallingManager` itself. `VcrToolCallingManager` wraps it, a new independent `VcrToolExecutionTrack` fixture type stores each tool invocation keyed by name + exact argument string (no change to `VcrTrack`'s schema or hash), and a new `VcrToolMode` axis — orthogonal to `VcrScope` — defaults to `REPLAY_FROM_CASSETTE` (full isolation); `EXECUTE_REAL` (opt-in via `@VcrTool`) recovers the old always-re-runs contract | **M** | Verified against a real model in `OllamaToolIsolationEndToEndTests`, through Spring AI's own `ToolCallingAutoConfiguration`/`ChatClientAutoConfiguration` — the one thing `OllamaToolCallingEndToEndTests` structurally cannot exercise, since it builds its `ChatClient` via the plain `ChatClient.builder(model)` factory with no Spring `ToolCallingManager` bean in play at all: the real tool runs once on record, zero times on an identical replay. See `docs/TOOL-ISOLATION-PRD.md` for the full diagnosis and design |
...
| **Tool / function calls** | **Verified against a real model.** `ToolDefinitionSnapshot` feeds the hash; `ToolCallSnapshot`/`ToolResponseSnapshot` (new in schema `"2"`) round-trip a message's own tool calls and tool results, not just the final response's; `VcrCacheKeyGenerator` now hashes a message's tool calls/responses explicitly instead of relying on `Message.getText()`, which is empty for both — the exact gap `OllamaToolCallingEndToEndTests` was written to close. `VcrScope`'s `INSIDE_TOOL_LOOP` vs `OUTSIDE_TOOL_LOOP` distinction is proven end to end: two model turns, two fixtures, zero network on replay. **Tool-call side-effect isolation, done** (`docs/TOOL-ISOLATION-PRD.md`): a real `@Tool` method with a side effect used to re-run on every replay under `INSIDE_TOOL_LOOP` — flagged as a genuine gap by an external review (`docs/EXTERNAL-FEEDBACK.md`). `VcrToolCallingManager` now wraps Spring AI's own `ToolCallingManager` bean (the one choke point every tool invocation passes through, confirmed via bytecode — not the `ToolCallback` interface Gemini's original proposal targeted, which a majority of real usage never registers as a bean) and defaults to `VcrToolMode.REPLAY_FROM_CASSETTE`: full isolation, the real method never runs on a hit. `EXECUTE_REAL` opts one test back into the old always-re-runs behaviour. Verified against a real model in `OllamaToolIsolationEndToEndTests`: the tool runs once on record, zero times on replay | None currently known |
